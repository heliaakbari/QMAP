...
from qiskit._accelerate.sabre import ha_routing, Heuristic, SetScaling, NeighborTable, SabreDAG
from qiskit._accelerate.nlayout import NLayout

logger = logging.getLogger(__name__)


class HASwap(TransformationPass):
    ...
    **References:**
    ...
    def __init__(self, coupling_map, heuristic="basic", seed=None, fake_run=False, trials=None):
        ...
        if isinstance(coupling_map, Target):
            ...
            if "cx" in self.target.keys():
                self.twoq_coupling_map = self.target.build_coupling_map("cx")
            elif "cz" in self.target.keys():
                self.twoq_coupling_map = self.target.build_coupling_map("cz")
            else:
                raise TranspilerError("the target has no cx or cz gates")

        else:
            raise TranspilerError("HA needs target properties")
        ...
        self.error_dist_matrix = None
        self.duration_dist_matrix = None
    ...
    def run(self, dag):
        ...
        self.dist_matrix = self.coupling_map.mix_dist_matrix
        ...
        *sabre_result, final_permutation = ha_routing(
            sabre_dag,
