...
use numpy::{PyReadonlyArray2, IntoPyArray, PyArray};
...
use super::haroute::{swap_map, swap_map_trial, RoutingTargetView};
...
) -> PyResult<PyObject> // Change return type to PyResult<PyObject> to return a list
{
    ...
    let seed_vec: Vec<u64> = (0..starting_layouts.len())
        .map(|_| outer_rng.gen())
        .collect();

    // The type of the result from layout_trial is inferred here as:
    // (NLayout, Vec<u32>, NodeBlockResults)
    let all_results: Vec<(NLayout, Vec<PhysicalQubit>, SabreResult)> = if run_in_parallel && starting_layouts.len() > 1 {
        seed_vec
            .into_par_iter()
            .enumerate()
    ...
    // Now, map the vector of Rust results into a PyList of Python tuples
    ...
    for res in all_results {
        ...
        // Append the Python tuple to the list
